...
object session {;import org.scalaide.worksheet.runtime.library.WorksheetSupport._; def main(args: Array[String])=$execute{;$skip(40); val res$0 = 
  ...
  def sqrtIter(guess: Double, x: Double): Double =
    ...
    else sqrtIter(improve(guess, x), x);System.out.println("""sqrtIter: (guess: Double, x: Double)Double""");$skip(90); 

  def isGoodEnough(guess: Double, x: Double) =
    abs(guess * guess - x) / x < 0.000001;System.out.println("""isGoodEnough: (guess: Double, x: Double)Boolean""");$skip(71); 
